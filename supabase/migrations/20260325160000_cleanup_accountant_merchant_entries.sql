-- Remove legacy accountant memberships mistakenly inserted on merchant companies
-- by older versions of accept_pending_invitations().

DELETE FROM public.user_companies uc
USING public.companies merchant, public.user_companies uc_own
WHERE uc.company_id = merchant.id
  AND merchant.accountant_company_id = uc_own.company_id
  AND uc_own.user_id = uc.user_id
  AND uc_own.role = 'accountant'
  AND uc.company_id != uc_own.company_id
