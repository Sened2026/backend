-- Link an existing accountant firm during merchant admin signup when
-- accountant_siren is present in auth metadata, and backfill historical rows.
...
WITH signup_candidates AS (
    SELECT
        u.id AS user_id,
        NULLIF(
            regexp_replace(COALESCE(u.raw_user_meta_data->>'accountant_siren', ''), '\D', '', 'g'),
            ''
        ) AS accountant_siren
    FROM auth.users u
    WHERE COALESCE(u.raw_user_meta_data->>'role', 'merchant_admin') = 'merchant_admin'
),
merchant_signup_companies AS (
    SELECT
        c.id AS company_id,
        sc.accountant_siren,
        row_number() OVER (
            PARTITION BY c.owner_id
            ORDER BY uc_owner.created_at ASC, c.created_at ASC, c.id ASC
        ) AS signup_company_rank
    FROM public.companies c
    JOIN signup_candidates sc
      ON sc.user_id = c.owner_id
    JOIN public.user_companies uc_owner
      ON uc_owner.company_id = c.id
     AND uc_owner.user_id = c.owner_id
     AND uc_owner.role = 'merchant_admin'
    WHERE c.accountant_company_id IS NULL
      AND sc.accountant_siren IS NOT NULL
      AND char_length(sc.accountant_siren) = 9
),
resolved_links AS (
    SELECT
        mc.company_id,
        accountant_company.id AS accountant_company_id
    FROM merchant_signup_companies mc
    JOIN public.companies accountant_company
      ON accountant_company.siren = mc.accountant_siren
    WHERE mc.signup_company_rank = 1
      AND accountant_company.id <> mc.company_id
      AND EXISTS (
          SELECT 1
          FROM public.user_companies uc
          WHERE uc.company_id = accountant_company.id
            AND uc.role = 'accountant'
      )
)
UPDATE public.companies c
SET accountant_company_id = resolved_links.accountant_company_id
FROM resolved_links
WHERE c.id = resolved_links.company_id
  AND c.accountant_company_id IS NULL;
