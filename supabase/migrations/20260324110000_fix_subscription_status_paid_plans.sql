-- Fix handle_new_user() trigger: paid plans should start as 'incomplete', not 'active'.
-- Only free plans (price_monthly = 0 AND price_yearly = 0) get 'active' status automatically.
-- This prevents users from bypassing Stripe payment during registration.
...
BEGIN
    ...
    -- Lire le plan choisi depuis les metadata
    ...
    -- Plans gratuits (price_monthly = 0 ET price_yearly = 0) → 'active' directement
    -- Plans payants → 'incomplete' (le paiement Stripe activera l'abonnement)
