...
export class CreateCompanyDto {
    ...
    @IsOptional()
    @IsString()
    @Matches(/^[0-9]{9}$/, { message: 'Le SIREN doit contenir exactement 9 chiffres' })
    siren?: string;
...
export type AccountantDocumentPeriodDto =
    ...
    | 'm11'
    | 'm12';
export type AccountantDocumentStatusDto = 'sent' | 'paid' | 'overdue' | 'cancelled';
...
const accountantDocumentStatuses = ['sent', 'paid', 'overdue', 'cancelled'] as const;
...
export class BulkDownloadLinkedClientDocumentsDto {
    ...
    @IsArray()
    @ArrayMaxSize(100)
    @IsUUID('4', { each: true, message: 'Chaque document_id doit être un UUID valide' })
    document_ids?: string[];
...
export class InviteAccountantFirmDto {
    @IsString()
    @MinLength(2, { message: 'Le nom du cabinet doit contenir au moins 2 caractères' })
    @MaxLength(255, { message: 'Le nom du cabinet ne peut pas dépasser 255 caractères' })
    firm_name: string;
