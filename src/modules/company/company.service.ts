...
type AccountantLinkRequestStatus = 'pending' | 'accepted' | 'rejected' | 'cancelled';
...
export class CompanyService {
     ...
     */
    async create(userId: string, createCompanyDto: CreateCompanyDto): Promise<CompanyWithRoleResponseDto> {
        const supabase = getSupabaseAdmin();
        const sourceAccountantCompanyId = createCompanyDto.source_accountant_company_id || null;
        ...
        if (sourceAccountantCompanyId) {
            const { data: sourceCompanyAccess, error: sourceCompanyError } = await supabase
                .from('user_companies')
        ...
        const { data: company, error: companyError } = await supabase
            ...
            .insert({
                ...
                default_vat_rate: createCompanyDto.default_vat_rate || 20.00,
        ...
        if (companyError) {
            throw new BadRequestException(`Erreur lors de la création: ${companyError.message}`);
        }
        ...
        const defaultRelationIds = (existingDefaultRelations || []).map((relation: { id: string }) => relation.id);
        ...
        if (defaultRelationIds.length > 0) {
            ...
            if (unsetDefaultError) {
                ...
                throw new BadRequestException(`Erreur lors de la mise à jour de l'entreprise par défaut: ${unsetDefaultError.message}`);
            }
        }
        ...
        if (relationError) {
            ...
            await supabase.from('companies').delete().eq('id', company.id);
            throw new BadRequestException(`Erreur lors de la liaison: ${relationError.message}`);
        }
     ...
     */
    async findAll(userId: string, query: CompanyQueryDto): Promise<CompanyListResponseDto> {
        const supabase = getSupabaseAdmin();
        ...
        const companyRelations = (relations || []).filter((rel: any) => rel.company);
     ...
     */
    async findOne(userId: string, companyId: string): Promise<CompanyWithRoleResponseDto> {
        const supabase = getSupabaseAdmin();
    ...
    async update(
        ...
        if (!canManageCompanyAsAdmin(accessContext.role, accessContext.companyOwnerRole)) {
            throw new ForbiddenException('Seuls les administrateurs peuvent modifier l\'entreprise');
        }
        ...
        if (updateCompanyDto.siren) {
            ...
            if (existingSiren) {
                throw new ConflictException('Une autre entreprise avec ce SIREN existe déjà');
            }
        }
     ...
     */
    async remove(userId: string, companyId: string): Promise<{ message: string }> {
        const supabase = getSupabaseAdmin();

        const accessContext = await this.checkUserAccessContext(userId, companyId);

        if (!canManageCompanyAsAdmin(accessContext.role, accessContext.companyOwnerRole)) {
            throw new ForbiddenException('Seuls les administrateurs peuvent supprimer l\'entreprise');
        }
     ...
     */
    async setDefault(userId: string, companyId: string): Promise<CompanyWithRoleResponseDto> {
        const supabase = getSupabaseAdmin();
        ...
        if (error) {
            throw new BadRequestException(`Erreur lors de la définition par défaut: ${error.message}`);
        }
    ...
    async updateGeneral(
        userId: string,
        companyId: string,
        data: { name?: string; legal_name?: string; siren?: string; vat_number?: string; logo_url?: string }
    ): Promise<CompanyWithRoleResponseDto> {
        const supabase = getSupabaseAdmin();
        ...
        const normalized = (data.siren !== undefined || data.vat_number !== undefined)
            ? normalizeBusinessIdentifiers({ siren: data.siren, vat_number: data.vat_number })
            : null;

        // Vérifie si le nouveau SIREN existe déjà (si modifié)
        const cleanedSiren = normalized?.siren ?? (data.siren === undefined ? undefined : null);
        if (cleanedSiren) {
            const { data: existingSiren } = await supabase
                .from('companies')
                .select('id')
        ...
        if (data.legal_name !== undefined) updateData.legal_name = data.legal_name || null;
        if (data.siren !== undefined) updateData.siren = normalized?.siren ?? null;
        if (data.vat_number !== undefined) updateData.vat_number = normalized?.vat_number ?? null;
    ...
    async updateContact(
        ...
        if (data.country !== undefined) {
            const { country } = normalizeBusinessIdentifiers({ country: data.country });
    ...
    async updateBanking(
        ...
        const updateData: any = {};
        if (data.rib_iban !== undefined) updateData.rib_iban = data.rib_iban || null;
        if (data.rib_bic !== undefined) updateData.rib_bic = data.rib_bic || null;
        if (data.rib_bank_name !== undefined) updateData.rib_bank_name = data.rib_bank_name || null;
    ...
    async updateSettings(
        ...
        const updateData: any = {};
        if (data.default_vat_rate !== undefined) updateData.default_vat_rate = data.default_vat_rate;
        if (data.default_payment_terms !== undefined) updateData.default_payment_terms = data.default_payment_terms;
        if (data.quote_validity_days !== undefined) updateData.quote_validity_days = data.quote_validity_days;
        if (data.terms_and_conditions !== undefined) updateData.terms_and_conditions = data.terms_and_conditions || null;
        if (data.quote_footer !== undefined) updateData.quote_footer = data.quote_footer || null;
        if (data.invoice_footer !== undefined) updateData.invoice_footer = data.invoice_footer || null;
    ...
    private async checkUserAccess(userId: string, companyId: string): Promise<CompanyRole> {
    ...
    private async getCompanyOwnerRoleMap(companyIds: string[]): Promise<Map<string, CompanyOwnerRole>> {
        ...
        if (ownerIds.length > 0) {
            const { data: ownerRelations, error: ownerRelationsError } = await supabase
                .from('user_companies')
            ...
            for (const relation of ownerRelations || []) {
                const company = (companies || []).find((entry: any) => entry.id === relation.company_id);
    ...
    private getCompanyInsertPayload(
        ...
        const { owner_role, source_accountant_company_id, ...companyData } = createCompanyDto;
        return companyData;
    }
    ...
    private async enrichWithAccountantInfo(
        ...
        if (accountantCompanyId) {
            ...
            companyResponse.accountant_link_status = pendingInvite ? 'invite_pending' : 'none';
        }
    ...
    private async getOwnedCompaniesCount(
        ...
        if (error) {
            throw new BadRequestException(`Erreur lors du calcul des entreprises possédées: ${error.message}`);
        }
    ...
    private async assertLinkedClientForAccountant(
        ...
        if (clientError || !clientCompany) {
            throw new NotFoundException('Client non trouvé ou non lié à votre cabinet');
        }
    }
    ...
    private validateAccountantDocumentStatuses(statuses?: string[]): AccountantDocumentStatus[] | undefined {
        if (!statuses || statuses.length === 0) {
            return undefined;
        }

        const uniqueStatuses = Array.from(new Set(statuses));
        if (uniqueStatuses.every((status) =>
            status === 'sent' || status === 'paid' || status === 'overdue' || status === 'cancelled'
        )) {
    ...
    private resolveAccountantDateRange(
        year?: number,
        period?: string,
    ): { year: number; period: AccountantDocumentPeriod; from: string; to: string } {
        ...
        const periods: Record<AccountantDocumentPeriod, { from: string; to: string }> = {
    ...
    private buildAccountantDownloadFilename(document: AccountantInvoiceListItem): string {
    ...
    private buildUniqueArchiveFilename(filename: string, usedNames: Map<string, number>): string {
        const normalizedFilename = this.sanitizeDownloadFilename(filename, 'document.pdf');
        const extensionIndex = normalizedFilename.lastIndexOf('.');
        const hasExtension = extensionIndex > 0;
        const baseName = hasExtension ? normalizedFilename.slice(0, extensionIndex) : normalizedFilename;
        const extension = hasExtension ? normalizedFilename.slice(extensionIndex) : '';
    ...
    private async loadAccountantDocuments(
        clientId: string,
        params: Pick<AccountantDocumentsQueryDto, 'type' | 'year' | 'period' | 'statuses'>,
    ): Promise<AccountantDocumentListItem[]> {
        const supabase = getSupabaseAdmin();
        const documentType = this.validateAccountantDocumentType(params.type || 'invoices');
        const documentStatuses = this.validateAccountantDocumentStatuses(params.statuses);
        ...
        return typedDocuments.map((invoice) => {
            ...
            return {
                ...invoice,
                document_kind: invoice.type === 'credit_note' ? 'credit_note' : 'invoice',
                ...
                downloadable_filename: storedDocument?.filename || this.buildAccountantDownloadFilename(invoice),
            };
        });
    }
    ...
    private async getStoredAccountantDocumentsMap(
        ...
        ((storedDocuments || []) as AccountantStoredDocumentRecord[]).forEach((storedDocument) => {
            if (!storedDocument.related_id || documentsMap.has(storedDocument.related_id)) {
                return;
            }
     ...
     */
    private async checkCompanyQuota(userId: string, ownerRole: OwnerCompanyRole): Promise<void> {
        const supabase = getSupabaseAdmin();
        ...
        const count = await this.getOwnedCompaniesCount(userId, ownerRole, {
            excludeMerchantCompaniesLinkedToAccountantCabinet: ownerRole === 'merchant_admin',
        });
    ...
    async linkAccountant(
        ...
        if (userRole !== 'merchant_admin') {
            throw new ForbiddenException('Seul un administrateur commerçant peut lier un expert-comptable');
        }

        if (companyId === accountantCompanyId) {
            throw new BadRequestException('Une entreprise ne peut pas être son propre expert-comptable');
        }
        ...
        if (error) {
            throw new BadRequestException(`Erreur lors de la liaison: ${error.message}`);
        }
    ...
    async unlinkAccountant(
        ...
        const userRole = await this.checkUserAccess(userId, companyId);
        if (userRole !== 'merchant_admin') {
            throw new ForbiddenException('Seul un administrateur commerçant peut délier un expert-comptable');
        }
    ...
    async inviteAccountantFirm(
        ...
        | { status: 'existing_accountant'; accountant_company: { id: string; name: string; siren: string | null } }
    ...
    > {
        ...
        const userRole = await this.checkUserAccess(userId, companyId);
        if (userRole !== 'merchant_admin') {
            throw new ForbiddenException('Seul un administrateur commerçant peut inviter un cabinet comptable');
        }
        ...
        const existingMatch = await this.findExistingAccountantCompany(normalizedSiren, normalizedEmail);
        if (existingMatch) {
            return {
        ...
        if (pendingInviteByEmail) {
            throw new ConflictException('Une invitation cabinet est déjà en attente pour cet email');
        }
        ...
        if (pendingInviteBySiren) {
            throw new ConflictException('Une invitation cabinet est déjà en attente pour ce SIREN');
        }
        ...
        if (invitationError || !invitation) {
            throw new BadRequestException(`Erreur lors de l'invitation du cabinet: ${invitationError?.message || 'inconnue'}`);
        }
        ...
        const inviterName = [inviter?.first_name, inviter?.last_name].filter(Boolean).join(' ') || 'Un administrateur';
    ...
    async getLinkedClients(userId: string, companyId: string): Promise<any[]> {
        ...
        if (!ACCOUNTANT_ROLES.includes(userRole)) {
            throw new ForbiddenException('Seuls les comptables peuvent voir les clients liés');
        }
        ...
        const [invoicesRes, quotesRes, creditNotesRes] = await Promise.all([
            supabase
                .from('invoices')
                .select('company_id, total, amount_paid, status, due_date, issue_date, type')
        ...
        return data.map((company: any) => {
            ...
            const companyQuotes = quotes.filter((q: any) => q.company_id === company.id);
            const companyCreditNotes = creditNotes.filter((cn: any) => cn.company_id === company.id);

            const annualInvoices = companyInvoices.filter(
                (i: any) => i.issue_date >= startOfYear && i.issue_date <= endOfYear
                    && ['sent', 'paid', 'overdue'].includes(i.status)
            );
            const annualRevenue = annualInvoices.reduce((sum: number, i: any) => sum + Number(i.total), 0);
            const totalPaid = companyInvoices.reduce((sum: number, i: any) => sum + Number(i.amount_paid || 0), 0);

            const overdueInvoices = companyInvoices.filter(
                (i: any) => ['sent', 'overdue'].includes(i.status) && i.due_date < today
            );
            const overdueAmount = overdueInvoices.reduce(
                (sum: number, i: any) => sum + Number(i.total) - Number(i.amount_paid || 0), 0
            );

            const pendingInvoices = companyInvoices.filter(
                (i: any) => ['sent'].includes(i.status)
            );
    ...
    async getAccountantLinkRequests(
        ...
        if (direction === 'incoming' && userRole !== 'merchant_admin') {
            throw new ForbiddenException('Seul un administrateur commerçant peut consulter les demandes de liaison entrantes');
        }

        if (direction === 'outgoing' && userRole !== 'accountant') {
            throw new ForbiddenException('Seul un expert-comptable administrateur peut consulter les demandes de liaison sortantes');
        }

        const column = direction === 'incoming' ? 'merchant_company_id' : 'accountant_company_id';
        const { data, error } = await supabase
            .from('accountant_link_requests')
            .select('id, accountant_company_id, merchant_company_id, requested_by, status, created_at, responded_at, responded_by')
            .eq(column, companyId)
            .eq('status', 'pending')
            .order('created_at', { ascending: false });

        if (error) {
            throw new BadRequestException(`Erreur lors du chargement des demandes de liaison: ${error.message}`);
        }

        return this.hydrateAccountantLinkRequests((data || []) as AccountantLinkRequestEntity[]);
    }
    ...
    async searchMerchants(
        ...
        if (userRole !== 'accountant') {
            throw new ForbiddenException('Seul un expert-comptable administrateur peut rechercher des commerçants');
        }
        ...
        if (merchantIdsError) {
            throw new BadRequestException(`Erreur lors de la recherche des commerçants: ${merchantIdsError.message}`);
        }

        const companyIds = Array.from(new Set((merchantCompanyIds || []).map((relation: any) => relation.company_id)))
        ...
        const pendingMerchantIds = new Set((pendingRequests || []).map((request: any) => request.merchant_company_id));

        const { data, error } = await supabase
            .from('companies')
            .select('id, name, legal_name, siren, email, city, logo_url, accountant_company_id')
            .in('id', companyIds)
            .is('accountant_company_id', null)
            .or(`name.ilike.%${searchTerm}%,legal_name.ilike.%${searchTerm}%,siren.ilike.%${searchTerm}%`)
            .limit(10);

        if (error) {
            throw new BadRequestException(`Erreur lors de la recherche des commerçants: ${error.message}`);
        }
    ...
    async createAccountantLinkRequest(
        ...
        if (userRole !== 'accountant') {
            throw new ForbiddenException('Seul un expert-comptable administrateur peut envoyer une demande de liaison');
        }

        if (accountantCompanyId === merchantCompanyId) {
            throw new BadRequestException('Une entreprise ne peut pas se demander une liaison à elle-même');
        }
        ...
        if (!merchantAdminRelation) {
            throw new NotFoundException('Cette entreprise n’est pas un commerçant éligible');
        }

        if (merchantCompany.accountant_company_id) {
            throw new ConflictException('Ce commerçant est déjà lié à un cabinet comptable');
        }
        ...
        if (existingPendingRequest) {
            throw new ConflictException('Une demande de liaison est déjà en attente pour ce commerçant');
        }
        ...
        if (error || !request) {
            throw new BadRequestException(`Erreur lors de la création de la demande de liaison: ${error?.message || 'inconnue'}`);
        }

        const [hydratedRequest] = await this.hydrateAccountantLinkRequests([request as AccountantLinkRequestEntity]);
    ...
    async acceptAccountantLinkRequest(
        ...
        return this.respondToAccountantLinkRequest(userId, merchantCompanyId, requestId, 'accepted');
    }
    ...
    async rejectAccountantLinkRequest(
        ...
        return this.respondToAccountantLinkRequest(userId, merchantCompanyId, requestId, 'rejected');
    }
    ...
    async getLinkedClientDocuments(
        ...
        const userRole = await this.checkUserAccess(userId, companyId);
        if (!ACCOUNTANT_ROLES.includes(userRole)) {
            throw new ForbiddenException('Seuls les comptables peuvent voir les documents des clients');
        }
        ...
        const downloadableTotal = documents.filter((document) => document.storage_available).length;
    ...
    async downloadLinkedClientDocumentsZip(
        ...
        const userRole = await this.checkUserAccess(userId, companyId);
        if (!ACCOUNTANT_ROLES.includes(userRole)) {
            throw new ForbiddenException('Seuls les comptables peuvent télécharger les documents des clients');
        }
        ...
        if (selectedIds.length > 100) {
            throw new BadRequestException('Le téléchargement groupé est limité à 100 documents');
        }

        const allFilteredDocuments = await this.loadAccountantDocuments(clientId, params);
        const selectedIdSet = new Set(selectedIds);
        const selectedDocuments = selectedIds.length > 0
            ? allFilteredDocuments.filter((document) => selectedIdSet.has(document.id))
            : allFilteredDocuments;

        const skippedLines: string[] = [];
        if (selectedIds.length > 0) {
            const foundIds = new Set(selectedDocuments.map((document) => document.id));
            selectedIds.forEach((documentId) => {
                if (!foundIds.has(documentId)) {
                    skippedLines.push(`Document sélectionné introuvable ou hors filtre: ${documentId}`);
                }
            });
        }

        const downloadableDocuments = selectedDocuments.filter((document) => document.storage_available);
        if (selectedIds.length === 0 && downloadableDocuments.length > 100) {
            throw new BadRequestException('Trop de documents archivés correspondent à ces filtres. Affinez votre recherche pour rester sous 100 documents.');
        }
        ...
        if (selectedDocuments.length === 0 || downloadableDocuments.length === 0) {
            throw new BadRequestException('Aucun document archivé disponible pour le téléchargement groupé');
        }
        ...
        const archiveClientName = this.sanitizeDownloadFilename(clientCompany?.name || 'client', 'client');
        const archiveFilename = `documents-${archiveClientName}-${archiveYear}-${archivePeriod}.zip`;
        const usedNames = new Map<string, number>();

        let firstSuccessfulEntry: { filename: string; buffer: Buffer } | null = null;
        ...
        for (let index = 0; index < downloadableDocuments.length; index += 1) {
            ...
            if (!storedDocument?.storage_path) {
                skippedLines.push(`Document ignoré (PDF archivé introuvable): ${document.invoice_number}`);
                continue;
            }
            ...
            if (downloadError || !fileData) {
                skippedLines.push(`Document ignoré (${document.invoice_number}): ${downloadError?.message || 'PDF introuvable dans le storage'}`);
                continue;
            }

            firstSuccessfulEntry = {
                filename: this.buildUniqueArchiveFilename(
                    storedDocument.filename || document.downloadable_filename || this.buildAccountantDownloadFilename(document),
                    usedNames,
                ),
        ...
        archive.append(firstSuccessfulEntry.buffer, { name: firstSuccessfulEntry.filename });

        for (let index = firstSuccessfulIndex + 1; index < downloadableDocuments.length; index += 1) {
            const document = downloadableDocuments[index];
            const storedDocument = storedDocumentsMap.get(document.id);
            if (!storedDocument?.storage_path) {
                skippedLines.push(`Document ignoré (PDF archivé introuvable): ${document.invoice_number}`);
        ...
        const unavailableSelectedDocuments = selectedDocuments.filter((document) => !document.storage_available);
        unavailableSelectedDocuments.forEach((document) => {
            skippedLines.push(`Document ignoré (PDF non archivé): ${document.invoice_number}`);
        });

        if (skippedLines.length > 0) {
            archive.append(Buffer.from(skippedLines.join('\n'), 'utf-8'), { name: 'erreurs.txt' });
        }
    ...
    async downloadLinkedClientDocument(
        ...
        const storedDocuments = await this.getStoredAccountantDocumentsMap(clientId, [invoice.id]);
        const storedDocument = storedDocuments.get(invoice.id);

        if (!storedDocument?.storage_path) {
            throw new NotFoundException('Le PDF immuable n’est pas disponible pour ce document');
        }
        ...
        return {
            buffer: Buffer.from(await fileData.arrayBuffer()),
            filename: storedDocument.filename || this.buildAccountantDownloadFilename(invoice),
    ...
    async validateInviteToken(token: string): Promise<{
        ...
        const { data: invitation, error } = await supabase
            .from('company_invitations')
            .select('company_id, email, role, invitation_type, expires_at, invited_by, invited_firm_name, invited_firm_siren, company:companies(name)')
            .eq('token', token)
            .is('accepted_at', null)
            .gt('expires_at', new Date().toISOString())
            .maybeSingle();
        ...
        return {
            ...
            invitation_type: ((invitation as any).invitation_type || 'member') as 'member' | 'accountant_firm',
    ...
    async getMembers(userId: string, companyId: string): Promise<any[]> {
        ...
        let quota: { max_members: number | null; current_members: number; pending_invitations: number } | null = null;
    ...
    async inviteMember(
        ...
        if (!canInviteRequestedSuperadmin) {
            this.ensureCanManageMembers(accessContext.role, accessContext.companyOwnerRole);
        }

        if (
            !canInviteRequestedSuperadmin
            && !getInvitableRolesForCompanyType(accessContext.companyOwnerRole).includes(role)
        ) {
            throw new ForbiddenException(
        ...
        if (existingUser) {
            ...
            if (existingMember) {
                throw new ConflictException('Cet utilisateur est déjà membre de cette entreprise');
            }
        }
        ...
        if (error) {
            throw new BadRequestException(`Erreur lors de l'invitation: ${error.message}`);
        }
        ...
        if (existingUser) {
            ...
            if (memberInsertError) {
                throw new BadRequestException(`Erreur lors de l'ajout du membre: ${memberInsertError.message}`);
            }
            ...
            if (invitationAcceptError) {
                ...
                throw new BadRequestException(`Erreur lors de la validation de l'invitation: ${invitationAcceptError.message}`);
            }

            if (this.shouldSyncMemberQuantity(accessContext.companyOwnerRole, role)) {
                try {
        ...
        // Get company and inviter info for the email
        const { data: company } = await supabase
            .from('companies')
            .select('name, email, phone, address, postal_code, city, siren, logo_url')
            .eq('id', companyId)
    ...
    async cancelInvitation(
        ...
        const { data: invitationToCancel, error: invitationError } = await supabase
            .from('company_invitations')
            .select('id, company_id, email, role, token, invited_by, expires_at, invitation_type')
            .eq('id', invitationId)
            .eq('company_id', companyId)
            .maybeSingle();
    ...
    private async respondToAccountantLinkRequest(
        ...
        if (userRole !== 'merchant_admin') {
            throw new ForbiddenException('Seul un administrateur commerçant peut répondre à une demande de liaison');
        }
        ...
        if (request.status !== 'pending') {
            throw new ConflictException('Cette demande de liaison a déjà été traitée');
        }
        ...
        if (decision === 'accepted') {
            ...
            if (merchantCompany.accountant_company_id) {
                throw new ConflictException('Cette entreprise est déjà liée à un cabinet comptable');
            }
            ...
            if (linkError) {
                throw new BadRequestException(`Erreur lors de l’acceptation de la liaison: ${linkError.message}`);
            }
        }
        ...
        if (updateError || !updatedRequest) {
            throw new BadRequestException(`Erreur lors de la mise à jour de la demande de liaison: ${updateError?.message || 'inconnue'}`);
        }
        ...
        const [hydratedRequest] = await this.hydrateAccountantLinkRequests([updatedRequest as AccountantLinkRequestEntity]);
        return hydratedRequest;
    }
    ...
    private async hydrateAccountantLinkRequests(
        ...
        if (error) {
            throw new BadRequestException(`Erreur lors du chargement des sociétés liées aux demandes: ${error.message}`);
        }
    ...
    private shouldSyncMemberQuantity(
        ...
        return companyOwnerRole === 'merchant_admin'
